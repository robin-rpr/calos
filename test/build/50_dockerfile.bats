...
@test 'Dockerfile: clearly image modify -c: validate tree' {
  ...
  -1  -1           IMAGE_REF foo
  -1  -1      shell
  -1  -1         STRING_QUO "/bin/sh"
  -1  -1         STRING_QUO "-c"
  -1  -1      run
  -1  -1        run_shell
  -1  -1           LINE_CHUNK echo foo
  -1  -1      run
  -1  -1        run_shell
  -1  -1           LINE_CHUNK echo bar
EOF
)

    diff -u <(echo "$blessed_tree") <(echo "$tree")
    echo "$output"
    [[ $status -eq 0 ]]
  ...
  -1  -1      copy
  -1  -1        copy_shell
  -1  -1           WORD /tmp/tmp_script.sh
  -1  -1           WORD /ch/script.sh
  -1  -1      run
  -1  -1        run_shell
  -1  -1           LINE_CHUNK /bin/sh /ch/script.sh
EOF
)
