...
@test 'mountns id differs' {
    ...
    echo "host:  ${host_ns}"
    guest_ns=$(clearly run "$clearly_timg" -- stat -Lc %i /proc/self/ns/mnt)
    echo "guest: ${guest_ns}"
...
@test 'userns id differs' {
    ...
    echo "host:  ${host_ns}"
    guest_ns=$(clearly run "$clearly_timg" -- stat -Lc %i /proc/self/ns/user)
    echo "guest: ${guest_ns}"
    [[ -n $host_ns && -n $guest_ns && $host_ns -ne $guest_ns ]]
}
...
@test 'distro differs' {
    ...
    guest_distro=$(clearly run "$clearly_timg" -- \
                          cat /etc/os-release \
                   | grep -F PRETTY_NAME \
                   | sed -r 's/^(.*")?(.+)(")$/\2/')
...
@test 'user and group match host' {
    scope full
    host_uid=$(id -u)
    guest_uid=$(clearly run "$clearly_timg" -- id -u)
    [[ $host_uid = "$guest_uid" ]]
    host_pgid=$(id -g)
    guest_pgid=$(clearly run "$clearly_timg" -- id -g)
    [[ $host_pgid = "$guest_pgid" ]]
    host_username=$(id -un)
    guest_username=$(clearly run "$clearly_timg" -- id -un)
    [[ $host_username = "$guest_username" ]]
    host_pgroup=$(id -gn)
    guest_pgroup=$(clearly run "$clearly_timg" -- id -gn)
