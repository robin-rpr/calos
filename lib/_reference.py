...
""" % tuple(  [str(self), self.for_path]
   ...
   def defaults_add(self):
      ...
      if (self.host is None):
         if ("CLEARLY_REGY_DEFAULT_HOST" not in os.environ):
            self.host = "registry-1.docker.io"
         else:
            self.host = os.getenv("CLEARLY_REGY_DEFAULT_HOST")
            self.port = int(os.getenv("CLEARLY_REGY_DEFAULT_PORT", 443))
            prefix = os.getenv("CLEARLY_REGY_PATH_PREFIX")
